...
Domain::Usuario SqliteUsuarioRepository::rowToUsuario(sqlite3_stmt* stmt) const {
    ...
    u.nombre = col(1);
    u.email  = col(2);
    if (sqlite3_column_type(stmt, 3) != SQLITE_NULL) u.fechaRegistro      = col(3);
    if (sqlite3_column_type(stmt, 4) != SQLITE_NULL) u.fechaUltimoBackup  = col(4);
    return u;
}
...
std::vector<Domain::Usuario> SqliteUsuarioRepository::getAll() const {
    ...
    const char* sql = "SELECT id, nombre, email, fecha_registro, fecha_ultimo_backup "
                      "FROM usuarios;";
...
bool SqliteUsuarioRepository::insert(const Domain::Usuario& u) {
    sqlite3_stmt* stmt = nullptr;
    const char* sql =
        "INSERT INTO usuarios (id, nombre, email, fecha_registro, fecha_ultimo_backup) "
        "VALUES (?, ?, ?, ?, ?);";
...
bool SqliteUsuarioRepository::update(const Domain::Usuario& u) {
    sqlite3_stmt* stmt = nullptr;
    const char* sql =
        "UPDATE usuarios SET nombre=?, email=?, fecha_registro=?, fecha_ultimo_backup=? "
        "WHERE id=?;";
    ...
    if (u.fechaRegistro)
        sqlite3_bind_text(stmt, 3, u.fechaRegistro->c_str(), -1, SQLITE_STATIC);
    else
        sqlite3_bind_null(stmt, 3);

    if (u.fechaUltimoBackup)
        sqlite3_bind_text(stmt, 4, u.fechaUltimoBackup->c_str(), -1, SQLITE_STATIC);
    else
        sqlite3_bind_null(stmt, 4);

    sqlite3_bind_text(stmt, 5, u.id.c_str(), -1, SQLITE_STATIC);
